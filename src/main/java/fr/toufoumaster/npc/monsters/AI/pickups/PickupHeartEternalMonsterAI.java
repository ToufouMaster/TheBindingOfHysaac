...
public class PickupHeartEternalMonsterAI extends PickupMonsterAI {
    ...
    public void onPlayerContact(Store<EntityStore> store, Ref<EntityStore> ref, PlayerRef playerRef) {
        ...
        if (healthBar.hasAngelicHeart) {
            healthBar.hasAngelicHeart = false;
            // Add red heart and or replace other heart if no space.
            healthBar.addHeart(HeartType.RedHeart);

        } else {
            healthBar.hasAngelicHeart = true;
        }
