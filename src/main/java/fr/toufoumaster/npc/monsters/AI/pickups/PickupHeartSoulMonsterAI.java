...
public class PickupHeartSoulMonsterAI extends PickupMonsterAI {

    int heart;

    public PickupHeartSoulMonsterAI(int heart) {
        super();
        this.heart = heart;
    }
    ...
    public void onPlayerContact(Store<EntityStore> store, Ref<EntityStore> ref, PlayerRef playerRef) {
        ...
        while (remaining > 0 && healthBar.getHearts().size() < 12) {
            int fullness = Math.min(remaining, HeartType.SoulHeart.getMaxFullness());
            healthBar.addHeart(HeartType.SoulHeart).setFullness(fullness);
            remaining -= fullness;
            canRecover = true;
        }
    ...
    public String getModel() {
        return switch (this.heart) {
            case 1 -> "Pickup_Heart_Soul_Half";
            case 4 -> "Pickup_Heart_Soul_Double";
            default -> "Pickup_Heart_Soul";
        };
    }
}
