...
public class PooterMonsterAI extends MonsterAI {

    private float lastAttack;
    public PooterMonsterAI() {
        super();
        lastAttack = 0f;
    }
    ...
    public void tick(float delta, Store<EntityStore> store, Ref<EntityStore> ref) {
        ...
        Vector3d targetPosition = IsaacMonsterComponent.getNearestTarget(store,  transformComponent.getPosition().clone(), 7);
        ...
        final float ATTACK_COOLDOWN = 2.5f;//50;
        ...
        if (this.getTimeAlive() > lastAttack+ATTACK_COOLDOWN) {
            npcEntity.playAnimation(ref, AnimationSlot.Status, "Shoot", store);
